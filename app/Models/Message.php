...
class Message extends Model {
    ...
    public static function store(Request $request) {
        ...
        Meta::incrementMessagesReceived(1);
    }

    public static function getMessages($email) {
        $limit = config('app.settings.fetch_messages_limit');
        $messages = Message::where('to', $email)->orWhere('to', 'like', '%' . $email . ',%')->orWhere('to', 'like', '%<' . $email . '>%')->latest()->take($limit)->get();
        $response = [
            'data' => [],
            'notifications' => []
        ];
        foreach ($messages as $message) {
            ...
            if ($message->attachments && !$blocked) {
                ...
                foreach ($attachments as $id => $attachment) {
                    ...
                    if (strpos($obj['content'], $attachment->{'content-id'}) !== false) {
