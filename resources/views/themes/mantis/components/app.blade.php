<main class="flex-1 border border-gray-200 m-5 md:m-0" x-data="{ show: false, id: 0 }">
    @if($error)
    <div id="imap-error" class="flex items-center w-full h-full fixed top-0 left-0 bg-red-900 opacity-75 z-50">
        <div class="flex flex-col mx-auto">
            ...
            <div class="p-2 mx-auto bg-red-800 text-white leading-none flex lg:inline-flex" role="alert">
                <span class="flex bg-red-500 uppercase px-2 py-1 text-xs font-bold mr-3">{{ __('Error') }}</span>
    ...
    <div class="text-sm">
        @if ($messages)
            <div class="mailbox">
                <div x-show="!show" class="list">
                    <div class="head flex items-center gap-3 pt-5 pb-6 px-7 text-white" style="background-color: {{ config('app.settings.colors.primary') }}">
                    ...
                    <div class="messages flex flex-col justify-end min-h-tm-mantis-message">
                ...
                <div x-show="show" class="message">
                    <div class="head flex items-center text-white py-5 px-7" style="background-color: {{ config('app.settings.colors.primary') }}">
                    ...
                    @foreach ($messages as $message)
                        <div x-show="id === {{ $message['id'] }}" id="message-{{ $message['id'] }}"
                            class="message min-h-tm-mantis-message">
                            <textarea
                            ...
                            <div class="text-wrap py-4 px-7">
                                <iframe class="w-full flex flex-grow min-h-tm-mantis-iframe"
                                ...
                                @if (count($message['attachments']) > 0)
                                    ...
                                    <div class="flex pb-5 px-6">
                                        @foreach ($message['attachments'] as $attachment)
                                            <a class="py-2 px-3 mr-3 text-sm border-2 border-black hover:bg-black hover:text-white"
        ...
        @else
            <div class="flex items-center min-h-tm-mantis-message">
